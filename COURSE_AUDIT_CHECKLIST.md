...
## Deferred future-cohort revision: burn-in and ensembles

Do not apply this revision to the current published cohort. Implement it as one
coordinated change to the unit roadmap, weekly signposting, Reader, slides and
workshops before the next cohort.

### Curriculum allocation

- [ ] Keep Week 6 modelling practice as **represent and explore heterogeneity**.
      Use variation in natural frequencies to show how a distribution assigned
      to individuals changes both individual and collective behaviour.
- [ ] Keep ensemble computation in Week 6 only where it supports the
      heterogeneity comparison. Do not make ensemble reporting the week's
      modelling-practice focus.
- [ ] Develop **burn-in and ensemble adequacy** in Week 8, alongside code
      verification and finite-size comparison.
- [ ] Preserve the earlier progression: Week 1 introduces independent runs;
      Week 5 uses them in parameter sweeps; Week 8 asks how many runs are
      adequate for a stated claim.

### Burn-in distinction to teach in Week 8

- [ ] Distinguish the loading transient from the stationary sequence of driven
      sandpile events and from the fast toppling dynamics within one avalanche.
- [ ] State that burn-in depends on the measurement question. Exclude initial
      loading when estimating stationary avalanche statistics; retain it when
      studying how the driven state develops.
- [ ] Test burn-in separately for each lattice size. Express driving time in
      additions per site, inspect full histories and compare later measurement
      windows of equal duration.
- [ ] Do not treat fast within-avalanche relaxation as burn-in. Avalanche size,
      area and duration are event measurements.

### Files and signposting to update together

- [ ] In `notebooks/week00/Getting_Started.ipynb`, make the roadmap assign
      heterogeneity to Week 6 and burn-in plus ensemble adequacy to Week 8.
- [ ] In `notebooks/week06/L_Synchronisation.ipynb` and
      `notebooks/week06/WS_Synchronisation.ipynb`, signpost heterogeneity as the
      modelling practice. Retain only the burn-in detail needed to interpret
      the current late-time measurements.
- [ ] In `notebooks/week08/L_Critical_phenomena.ipynb`, signpost verification,
      burn-in and ensemble adequacy as modelling practice. Keep finite-size
      comparisons, spatial correlations and multiple time scales as toolkit
      additions.
- [ ] In `notebooks/week08/WS_Critical_phenomena.ipynb`, require a visible
      burn-in check before the finite-size ensemble: full loading histories,
      equal-duration later windows and a separately justified choice for each
      lattice size.
- [ ] Update `TEACHING_GUIDE.md`, `glossary.md`, practice questions and future
      instructor checks to match the revised allocation.
- [ ] Rebuild and inspect both weeks together. Confirm that the Week 6 pathway
      remains centred on heterogeneity and the Week 8 investigation distinguishes
      transient, stationary and within-event dynamics before pooling runs.
...
- **2026-08-26:** Week 10 has one canonical game, the Prisoner's Dilemma.
  ...
  canonical model.
- **2026-09-14:** Defer the burn-in curriculum revision until the next cohort.
  Week 6 will retain heterogeneity as modelling practice. Week 8 will develop
  burn-in and ensemble adequacy through the sandpile's loading transient,
  stationary event sequence and within-avalanche relaxation.
