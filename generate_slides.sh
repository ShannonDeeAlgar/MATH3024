...
cleanup() {
    rm -rf "$KERNEL_ROOT"
    if [[ -n "$RESOLVED_NOTEBOOK" ]]; then
        rm -f "$RESOLVED_NOTEBOOK" "${RESOLVED_NOTEBOOK%.ipynb}.slides.html"
    fi
...
RESOLVED_NOTEBOOK="$NOTEBOOK_DIR/.${NOTEBOOK_NAME}.resolved.$$.ipynb"
"$COURSE_PYTHON" "$SCRIPT_DIR/tools/check_equation_consistency.py" \
    --notebook "$SOURCE" --materialize "$RESOLVED_NOTEBOOK"
INPUT="$RESOLVED_NOTEBOOK"
if [[ "$FRESH" -eq 1 ]]; then
    NOTEBOOK_DIR="$(cd "$(dirname "$SOURCE")" && pwd)"
    NOTEBOOK_NAME="$(basename "$SOURCE" .ipynb)"
