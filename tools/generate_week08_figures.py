...
def add_and_relax(pile, rng, record_frames=False):
    ...
    while np.any(pile >= 4):
        # One parallel relaxation step gives every currently unstable site
        # one legal toppling.  A site that remains unstable, or becomes
        # unstable through incoming load, topples again on the next step.
        active = pile >= 4
        mask |= active
        pile[active] -= 4
        topplings += int(active.sum())
        duration += 1
        pile[1:, :] += active[:-1, :]
        pile[:-1, :] += active[1:, :]
        pile[:, 1:] += active[:, :-1]
...
def main():
    ...
    if avalanche_frames:
        ...
        # Stable loads 0--3 retain their own colours.  Every load at or above
        # the instability threshold uses a fifth colour.  ``clip=True`` makes
        # the final class open-ended, so 4, 5, ... are all visibly unstable.
        cmap = ListedColormap(["#F7F8FB", BLUE, YELLOW, CORAL, PURPLE])
        norm = BoundaryNorm(np.arange(-0.5, 5.5, 1), cmap.N, clip=True)
    ...
    # Use the largest symmetric pile for the height-class comparison. Keep the
    # masks visually simple; the adjacent box-counting plot carries the fits.
    symmetric = fields[-1]
    level_colours = ["#F7F8FB", BLUE, YELLOW, CORAL]
    fig, axes = plt.subplots(2, 2, figsize=(6.2, 5.8), constrained_layout=True)
    for level, (ax, colour) in enumerate(zip(axes.flat, level_colours)):
        ...
        ax.set_title(f"Height {level}", pad=5)
        ax.set_xticks([]); ax.set_yticks([])
    fig.savefig(OUT / "sandpile_height_subsets.png", dpi=220, transparent=True)
    plt.close(fig)
    ...
    for level, color in zip(range(4), [NAVY, BLUE, CORAL, YELLOW]):
        scales, counts = box_counts(symmetric == level)
        ax.loglog(1/scales, counts, "o-", color=color,
                  label=f"height {level}")
    ...
    for row, (values, symbol, label, color) in enumerate([
        ...
        axes[row, 0].set(
            xlabel=f"Avalanche {label}, ${symbol}$",
        ...
        axes[row, 1].loglog(fit_x, 10**intercept * fit_x**slope, color=NAVY, lw=2.4,
                            label=f"local fit: slope {slope:.2f}")
        ...
        axes[row, 1].legend(frameon=False, fontsize=12.5, loc="best")
    ...
    style(ax)
    # This grain-addition diagnostic is not the Reader's internal-step spectrum.
    # The latter is generated with its matching audio by make_temporal_analysis.
    fig.savefig(OUT / "sandpile_grain_addition_spectrum.png", dpi=220, transparent=True)
    plt.close(fig)

    # Renormalisation map. Calculate, rather than position by eye, the
    # non-trivial fixed point of R(p) = p^2(2-p)^2.
    def phi(p):
    ...
    ax.plot(x, x, "--", color="#92A7C3", lw=2, label="$P_{m+1}=P_m$")
    ax.plot(x, phi(x), color=NAVY, lw=3, label="$P_{m+1}=R(P_m)$")
    p = 0.28
    for _ in range(12):
        q = phi(p)
    ...
    ax.annotate(f"unstable fixed point\n$P^*=(3-\\sqrt{{5}})/2\\approx{pc:.3f}$",
                ...
                fontsize=13)
    ax.set(xlim=(0, 1), ylim=(0, 1), xlabel="$P_m$", ylabel="$P_{m+1}$")
    style(ax)
